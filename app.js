(function () {
  ...
  // State
  let logoData = localStorage.getItem('pb_logo_data') || null;
  let logoImg = null;
  ...
  /* const gmail = { token: null, tokenClient: null, authorized: (localStorage.getItem('gmail_authorized') === '1') }; */
  ...
  async function startCam() {
    ...
    if (!isSecureContext && !isLocal()) { alert('Bitte über https:// oder http://localhost öffnen.'); return false; }
    try {
  ...
  function draw() {
    ...
    (function drawLogoOnCanvas(ctx, canvasW, canvasH) {
      ...
      if (logoImg && (logoImg.complete || logoImg.naturalWidth)) {
        ...
        ctx.globalAlpha = 1.0;
        ctx.drawImage(logoImg, canvasW - w - padX, padY, w, h);
      } else {
        ctx.globalAlpha = .9;
        ...
        ctx.fillText(text, canvasW - m.width - padX, padY + 54);
      }
      ctx.restore();
  ...
  async function shoot() {
    ...
    await waitReady();
    S.count.style.display = 'block'; for (let i = 5; i >= 1; i--) { S.count.textContent = String(i); await sleep(1000); }
    S.count.style.display = 'none'; flash(); await shutter(); draw();
    showScreen('captured');
  }
  ...
  async function compose(src) {
    ...
    // Logo oben rechts
    if (logoImg && (logoImg.complete || logoImg.naturalWidth)) {
      const padX = Math.round(W * 0.02); const padY = Math.round(H * 0.02);
      const maxW = Math.round(W * 0.18); const maxH = Math.round(H * 0.18);
      let w = logoImg.naturalWidth || logoImg.width; let h = logoImg.naturalHeight || logoImg.height;
      const s = Math.min(maxW / w, maxH / h, 1);
      w = Math.round(w * s); h = Math.round(h * s);
      ctx.drawImage(logoImg, W - w - padX, padY, w, h);
    } else {
      ctx.save(); ctx.globalAlpha = .25; ctx.fillStyle = '#0f172a';
      ctx.font = 'bold 32px ui-sans-serif,system-ui,-apple-system,Segoe UI,Roboto';
      const m = ctx.measureText('AI Hub'); ctx.fillText('AI Hub', W - m.width - 24, 24 + 32); ctx.restore();
    }
  ...
  async function toOpenAI(p) {
    ...
    try {
      ...
      if (!b64) throw new Error('Unerwartete Antwort');
      let url = 'data:image/png;base64,' + b64;
      url = await compose(url);
      S.resultBox.innerHTML = '';
      const img = new Image(); img.src = url; img.alt = 'Generiertes Bild'; img.style.maxWidth = '100%'; img.style.maxHeight = '100%';
      S.resultBox.appendChild(img); S.finalImg.src = url; showScreen('result');
    } catch (e) {
      try {
        ...
        const j2 = await r2.json(); const b642 = j2 && j2.data && j2.data[0] && j2.data[0].b64_json;
        if (!b642) throw new Error('Keine Bilddaten');
        let url2 = 'data:image/png;base64,' + b642; url2 = await compose(url2);
        S.resultBox.innerHTML = ''; const img2 = new Image(); img2.src = url2; S.resultBox.appendChild(img2); S.finalImg.src = url2; showScreen('result');
      } catch (err) {
        console.error(err);
  ...
  const loadClientId = async () => {
    ...
    if (meta && meta.content) { drive.clientId = meta.content.trim(); return drive.clientId; }
    return null;
  };
  const initToken = () => {
    ...
    return google.accounts.oauth2.initTokenClient({
      ...
      callback: (resp) => { if (resp && resp.access_token) { drive.token = resp.access_token; setDriveStatus(); } }
    });
  };

  /* ===== Gmail deaktiviert =====
  async function ensureGmailToken({ interactive = true } = {}) { ... }
  ...
  */
  ...
  // Silent token (keine UI), nur wenn früher autorisiert
  async function ensureToken(opts) {
    ...
    if (drive.authorized) { // silent
      try { drive.tokenClient.requestAccessToken({ prompt: 'none' }); } catch (e) { /* ignore */ }
      for (let i = 0; i < 20; i++) { if (drive.token) return drive.token; await sleep(100); }
    }
    if (interactive) {
      try { drive.tokenClient.requestAccessToken({ prompt: 'consent' }); } catch (e) { /* ignore */ }
      for (let i = 0; i < 50; i++) {
  ...
  async function createFolderIfNeeded() {
    if (driveFolder && driveFolder.id) return driveFolder;
